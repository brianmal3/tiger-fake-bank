...
#  ./pull.sh ~/.ssh/i_greene git@github.com:brianmal3/tiger-backend-app.git 


echo "🔴 🔴 🔴 🔴 🔴 Generic GitHub Pull script starting ..."
echo "🔴 🔴 🔴"

# Ensure the script is called with three arguments
if [ "$#" -ne 2 ]; then
  echo "👿 Please enter required parameters: SSH key path, repository SSH URL  👿"
  exit 1
fi
